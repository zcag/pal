...
1. **Nothing queued.** Weather, Power, Bluetooth, Network and Audio are all at
   parity. Network's compact kv popover and Audio's volume-level flash — the last
   two named — landed with this pass. What is left on the bar is new items (E13
   privacy, E14 tan, E15 claude) and the config-only rows, not polish.
...
| E9 | `audio/volume` + `audio/microphone` | **done:** output glyph/level, direct mute, wheel ±5, 18 pt / 31 pt stable glyph slot, Audio palette popover and 5 s poll; mic hidden while healthy, direct 75% restore when muted or absent. Polished to parity: the glyph carries a three-step level ramp with 0 as muted, and the percentage is *feedback* — it appears for three seconds after a scroll or a mute and then collapses back to the glyph (a `level` setting picks `flash` / `always` / `never`). The collapse is a `setTimeout` + `bar.refresh`, since `refresh.every` is floored at 10 s. The old "high" glyph was volume-*plus*, a latent bug, now the loudness ramp. Sound Settings action and a native audio event remain deferred. | M | ∥ |
