...
| volume | **missing** | `audio` reads output / input volume and mute (`core/src/audio.rs`) but has no bar item | `audio/output`: the route / level glyph, label `NN%` for 3 s after a change then title-less (a `refresh: 3` does the flash), click mutes (G4), scroll ±5 (G6), popover = the device rows switching output + system device and Sound Settings. Needs a change event: pal has no `volume_change`; a 5 s poll while shown or a CoreAudio listener in core (G8). Pinned icon width (G9). (M) |
| mic | **missing** | `audio` has the input device's `muted` flag | `audio/mic`: hidden unless input is muted or absent, `󰍭` / `󰍾` red, click sets 75 (G4). (S) |
...
| network | **missing** | `wifi` (CoreWLAN in-process: ssid, rssi, `core/src/wifi.rs:42`) and `network` (interfaces, gateway) have no bar item | `wifi/status`: hidden at home (`home_ssid`, `home_gateway` settings), `󰤭` red with no route, hotspot / public / RSSI ramp / wired glyphs, click opens Network Settings (G4), popover kv (interface, ip, gateway, signal, network); `refresh: 5`, `on: [network, wake]`. (M) |
...
| E9 | `audio/output` + `audio/mic` | output: glyph by route / level, `title` `NN%` only for 3 s after a change (`refresh: 3`, compare with the last level in memory), `click: "open"` → mute (B1), scroll (B6), `icon_width = 31` + `icon_size = 18` (B2), popover rows switching the default output **and** system device (`SwitchAudioSource -t system`, the reconcile at `volume.sh:102-110`), Sound Settings; mic: hidden unless muted / absent, `click: "open"` sets 75. Poll `refresh: 5` until B8. | M | ∥ |
...
| E11 | `wifi/status` | settings `home_ssid`, `home_gateway`; hidden at home; no default route → `󰤭` red; glyph by SSID pattern (hotspot / public) or RSSI ramp (`network.sh:80-92`), wired `󰈀`; `click: "open"` → Network Settings; popover kv; `refresh: 5`, `on: [network, wake]`. SSID comes from CoreWLAN in-process with Location (pal already asks), so the sudoers rule is not needed. | M | ∥ |
