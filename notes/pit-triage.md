...
| 120 | `01M2KJSVDHS83Z1K2NT67CA28K` | builtin: Move windows between displays and Spaces | partial | V:S E:M | Next / Previous Display are in. Spaces themselves are in since 2026-09-22 (the Spaces palette: list and switch, `windows.spaces` / `go_space`, "Decided: spaces"); moving a window to one is not (`SLSMoveWindowsToManagedSpace` may need the scripting addition), and Cagdas does not want it |
