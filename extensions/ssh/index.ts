// SSH hosts from ~/.ssh/config: every `Host` block that is a name rather than a pattern, with its HostName,
 ...
 * is followed only from the top file (one level).
 */
...
async function ping(target: string): Promise<{ ok: true; ms: number } | { ok: false; why: string }> {
  const { code, out, err } = await exec(["ping", "-c", "1", "-W", LINUX ? "2" : "2000", target], { ms: PING_MS });
