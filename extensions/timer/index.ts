...
export function barItem(ts: Timer[]): BarItem {
  ...
  // No timer: hidden; the glyph and the popover (open on the field, so a click starts one) are the `empty` shape a `show = "always"` config keeps, muted.
  if (!t) return { hidden: true, empty: { icon: GLYPH, tooltip: "No timers", menu } };
