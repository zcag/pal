...
async function systemFolder(): Promise<string> {
  ...
  const out = (await exec(["defaults", "read", "com.apple.screencapture", "location"]).then((r) => r.out, () => "")).trim();
...
type Entry = { path: string; name: string; kind: Kind; size: number; mtime: number; dims?: { width: number; height: number } };
...
const captureRows = (s: Settings): Item[] => {
  if (!MAC && !(Bun.which("grim") && Bun.which("slurp"))) return [hint("grim", "grim and slurp are not installed", "The capture rows need both on PATH (wl-copy for the clipboard)", { section: CAPTURE_SECTION })];
...
function row(e: Entry, section?: string): Item {
  const dims = e.dims ? `${e.dims.width}×${e.dims.height} · ` : "";
  return {
    id: e.path,
    name: e.name,
    subtitle: `${dims}${bytes(e.size)}`,
...
async function list(): Promise<Item[]> {
  ...
  const recent = entries.length ? entries.map((e) => row(e, RECENT_SECTION)) : [hint("empty", "No screenshots yet", `${tilde(dir)} has none; a capture above lands there`, { section: RECENT_SECTION })];
...
async function capture(c: Capture): Promise<void> {
  ...
  const { code, err } = await exec(argv, { ms: CAPTURE_MS });
...
const trash = (p: string) => run(process.env.PAL_SCREENSHOTS_TRASH ? [process.env.PAL_SCREENSHOTS_TRASH, p] : MAC ? ["osascript", "-e", `tell application "Finder" to delete POSIX file ${JSON.stringify(p)}`] : ["gio", "trash", "--", p], { ms: TRASH_MS });
