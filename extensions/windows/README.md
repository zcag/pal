...
`minimized` tag, one on another workspace or space `ws 3` or `other
space`, and the monitor when known.
...
| `cmd+h` | Hide app (macOS): System Events hides the window's process |
...
- No ranking: the order is the desktop's, so the window you used most is
  not first. Type part of its title instead.
