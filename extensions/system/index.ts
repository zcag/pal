...
import { bar, effects, errorMessage, failed, home, hint, now, settings, storage, system, toast, view as liveView, type Accessory, type Action, type Arg, type BarItem, type Effect, type Extension, type Form, type FormValues, type Item, type LinkParams, type SystemCommand } from "@zcag/pal";
  ...
  palettes: {
    [PALETTE]: {
      ...
      list: async () => {
        ...
        return all.flatMap((c) => (c.id === AWAKE ? [awakeRow(awake, s, now())] : c.available ? [item(c, s.confirm_destructive, probes)] : []));
      },
