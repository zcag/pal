...
| `root_first_step` | number | `25` | Points between two `root_first` palettes. The word bonus is 300 and the tier spread 150 (the ladder in `core/src/index.rs` under `EXACT_BONUS`), so keep the whole ladder (step times the list's length) under 150 for the rules above to hold. |
...
| `now` | list of strings | `["spotify-now-playing", "media", "calendar-today", "timer-timers", "clipboard-rows"]` | The palettes whose suggestions lead the empty root (the "Now" section: what is playing, the next event, the running timer; the "Clipboard" section: what is on the clipboard), in this order; suggesting palettes not named here follow in load order. |
...
registered. Settings > Shortcuts says "Not registered" with the OS's reason
...
| `alias` | string | unset | A short name for the palette. It is added as a keyword on the palette's row at the root, so typing it finds the palette; `Enter` opens it. It also names the palette's **rows**: a query whose other words match a row is scoped by it (`tod estonia` for a todo in Todos), where the alias alone still lists the palette itself. |
...
| `tier` | `primary`, `normal`, `catalog` | unset | The palette's tier at the root over what its manifest says: `primary` is ranked up and capped at `root_caps.primary` rows, `catalog` ranked down and capped at `root_caps.catalog` ([Extensions](extensions.md#tier-what-the-rows-are-at-the-root)). `tier = "catalog"` on a data-file palette whose rows flood the root; `tier = "primary"` on one you reach for by name; Settings › Extensions (a palette unfolded under its extension) has it as "At the root". |
| `panel` | `normal`, `big`, `corner` | unset (normal) | A game page's panel ([game surfaces](design/game-surface.md#sizes)): `big` is 90% of the screen, centred; `corner` is 560 by 380 in the screen's bottom-right corner. Written by the game level's own ⌘⇧F (Enlarge panel) and ⌘⇧J (Panel in the corner), so each game opens the way it was left; other palettes ignore it. |
...
| `settings` | table | `{}` | The item's own settings, the ones its manifest declares under `bar.<id>.settings` (Spotify's playing item's `lyrics`, a stats item's `label`), over their defaults; an instance's item (`"gmail@work/unread"`) inherits what the default instance's sets. Settings › Bar edits them on the item's pane. A change renders the item again. Before 2026-09-23 these were extension settings (tagged `bar`, or named `bar_*`); an older file's are moved here once at startup, as for the features above. |
