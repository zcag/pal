...
// Typed loosely: the layout request's `step` and the verbs added after `WindowLayout` are not in protocol.ts yet (another pass's file).
const pick = (id: string, action?: string, args?: unknown, palette = EXT) => host.pick(EXT, palette, id, action, args === undefined ? undefined : { args }) as Promise<unknown>;
...
describe("window-management", () => {
  test("meta: an indexed layout palette and an input window picker", () => {
    ...
    expect(l.manifest.settings?.map((s) => [s.id, s.default])).toEqual([["gap", 0], ["almost_maximize_percent", 90], ["reasonable_size_percent", 60], ["step", 32]]);
  });
  ...
  test("Apply is a layout effect carrying the settings' knobs, the focused window implied", async () => {
    expect(await pick("left_half")).toEqual({ layout: { name: "left_half", gap: 0, almost_maximize_percent: 90, reasonable_size_percent: 60, step: 32 } });
    expect(await pick("maximize", "apply")).toEqual({ layout: { name: "maximize", gap: 0, almost_maximize_percent: 90, reasonable_size_percent: 60, step: 32 } });
    host.changeSettings(EXT, { settings: { gap: 8, reasonable_size_percent: 50, step: 64 } });
    expect(await pick("restore")).toEqual({ layout: { name: "restore", gap: 8, almost_maximize_percent: 90, reasonable_size_percent: 50, step: 64 } });
    expect(await pick("move_left")).toEqual({ layout: { name: "move_left", gap: 8, almost_maximize_percent: 90, reasonable_size_percent: 50, step: 64 } });
    host.changeSettings(EXT, {});
    expect(await pick("unminimize")).toEqual({ layout: { name: "unminimize", gap: 0, almost_maximize_percent: 90, reasonable_size_percent: 60, step: 32 } });
    // No window to pick for it: Apply to… is Apply.
    expect(await pick("unminimize", "apply-to")).toEqual({ layout: { name: "unminimize", gap: 0, almost_maximize_percent: 90, reasonable_size_percent: 60, step: 32 } });
  ...
  test("Apply to… drills into arrange, whose rows are the open windows and whose pick applies to that window", async () => {
    ...
    expect(await pick("w3", "apply", { layout: "right_half" }, "arrange")).toEqual({ layout: { name: "right_half", id: "w3", gap: 0, almost_maximize_percent: 90, reasonable_size_percent: 60, step: 32 } });
  ...
  test("from the root, arrange picks a window first and lists the layouts for it", async () => {
    ...
    expect(await pick("top_half", "apply", { id: "w1", title: "~/proj/pal" })).toEqual({ layout: { name: "top_half", id: "w1", gap: 0, almost_maximize_percent: 90, reasonable_size_percent: 60, step: 32 } });
