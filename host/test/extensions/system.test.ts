...
// selection (`core/selection.files`) and front app (`core/states.get`), a
// stand-in for qlmanage (`PAL_FILES_QUICKLOOK`) that logs its paths, and a
// shell script standing in for `caffeinate` (`PAL_AWAKE_TOOL`) that logs its
...
afterAll(() => { host.kill(); rmSync(trash, { recursive: true, force: true }); rmSync(tools, { recursive: true, force: true }); rmSync(fake, { recursive: true, force: true }); });
...
describe("keep awake: the row, the bar item and the link", () => {
  ...
  test("off: the row replaces the core's in its place, with the fields in the bar and three actions; the bar item is hidden with the coffee as its empty shape", async () => {
    const items = await list();
    expect(items.map((i) => i.id)).toEqual(["sleep", "shutdown", "empty-trash", "dark-mode", "keep-awake", "quit-all", "unhide-all", ...(MAC ? ["dismiss-notifications", "quick-look-selection"] : [])]);
    const row = items[4];
