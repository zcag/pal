...
const INSTALLED = [...["games", "snake", "solitaire", "blackjack", "minesweeper", "yahtzee", "2048", "gifs", "calc"].map((n) => ext(n)), ext("wordle", false), { ...ext("gone"), root: "/nowhere" }];
...
test("one row per game: loaded, shelved under Fun, a view palette; by title, with the game's tile and tagline", async () => {
  ...
  expect(rows.map((r) => r.id)).toEqual(["2048/2048", "blackjack/blackjack", "minesweeper/minesweeper", "snake/snake", "solitaire/solitaire", "yahtzee/yahtzee"]);
