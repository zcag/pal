...
describe("tela", () => {
  ...
  describe("research", () => {
    ...
    test("asking: one MCP session (initialize, initialized, tools/call), the view with the flags, the sources numbered with the cursor on the first, its excerpt rendered under them; the question remembered", async () => {
      ...
      expect(t.slice(t.indexOf("(1)"), t.indexOf("(2)"))).toEqual(["(1)", "Indexing", "Engineering · Indexing › Steps · 11 h ago", "Indexing", "Every listing is persisted and restored, see Startup.", "↗ Startup", "[Note]", "Why persist", "400 ms cold."]);
