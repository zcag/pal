...
describe("tela", () => {
  ...
  describe("research", () => {
    ...
    test("asking: one MCP session (initialize, initialized, tools/call), the view with the flags, the sources numbered with the cursor on the first, its excerpt rendered under them; the question remembered", async () => {
      ...
      expect(t.slice(t.indexOf("(1)"), t.indexOf("(2)"))).toEqual(["(1)", "Indexing", expect.stringMatching(/^Engineering · Indexing › Steps · \d+ (min|h|d|w|mo|y) ago$/), "Indexing", "Every listing is persisted and restored, see Startup.", "↗ Startup", "[Note]", "Why persist", "400 ms cold."]);
