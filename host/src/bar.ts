...
import type { BarCtx, BarMeta, BarSource, Extension, Manifest } from "../../sdk/src/protocol.ts";
...
/** The core's `ctx` as the extension sees it; a request without one gets `reason` (a first render, a popover opening). */
const ctxOf = (p: any, reason: BarCtx["reason"] = "load"): BarCtx => (p?.ctx && typeof p.ctx === "object" && typeof p.ctx.reason === "string" ? p.ctx : { reason });

/** `bar/render`, `bar/action`, `bar/open`, `bar/shown`; `lookup` throws the load error for an extension that is not there. */
export function barMethods(lookup: (name: string) => Extension): Record<string, (params: any) => unknown> {
  ...
  return {
    "bar/render": async (p) => checkBarItem(await run(p, () => source(p).render(ctxOf(p))), `${key(p)}: render`),
    // `{}` when the item has no handler or answers nothing: the popover stays as it is.
    "bar/action": async (p) => checkEffect((await run(p, () => source(p).onAction?.(String(p?.action ?? ""), ctxOf(p)))) ?? {}, `${key(p)}: action ${p?.action ?? ""}`),
    "bar/open": async (p) => checkEffect((await run(p, () => source(p).onOpen?.(ctxOf(p)))) ?? {}, `${key(p)}: open`),
    // A notification: nothing answers it, a throw is a log line (host.ts).
    "bar/shown": async (p) => { await run(p, () => source(p).onShown?.(ctxOf(p, "open"))); },
  };
}
