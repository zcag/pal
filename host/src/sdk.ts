...
import { onStates } from "./states.ts";
...
export const bindSdk = () => bind({ call, caller, resolved, subscribe, update: (extension, s) => update({ [extension]: s }), views, onView, onStates, instance: (extension) => ({ key: extension, name: extension, isDefault: true }) });
