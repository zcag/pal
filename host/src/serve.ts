...
import type { Ctx, Extension, Item, Manifest, Palette } from "../../sdk/src/protocol.ts";
...
export function paletteMethods(lookup: Lookup, manifestOf: Manifests): Record<string, (params: any) => unknown> {
  const palette = (p: any) => {
    const pal = lookup(String(p?.extension)).palettes[p?.palette];
    if (!pal) throw new Error(`no palette ${paletteKey(p)}`);
    return pal;
  };
  const fix = <T>(r: T, p: any): T => rewriteEffect(r, String(p?.extension));
  return {
    list: async (p) => {
    ...
    pick: async (p) => fix(checkEffect((await inContext(p, () => palette(p).pick(p.id, p.action, ctxOf(p)))) ?? {}, `${paletteKey(p)}: pick ${p.action ?? ""}`), p),
    ...
    link: async (p) => {
      ...
      const params = checkLinkParams(spec, p?.params && typeof p.params === "object" ? p.params : {}, `${key}/${route}`);
