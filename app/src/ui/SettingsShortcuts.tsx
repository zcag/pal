...
export const text = {
  ...
  sidebar: { anchor: "shortcuts:sidebar", hint: "Engage the sidebar", label: "Sidebar", description: "Engages the sidebar from any app: shown key from hidden, or a peek made key. Which palette it docks, and where, is on General.", keywords: "sidebar edge hotkey engage" },
    ...
    <div className="pal-settings-page pal-shortcuts">
      ...
      {matches(filter, "show pal", ...general.hotkeys.map(comboLabel), "window switcher", chord && comboLabel(chord), "app switcher", general.appSwitcher && comboLabel(general.appSwitcher), "sidebar", sidebar?.value.hotkey && comboLabel(sidebar.value.hotkey)) && (
        <SettingsGroup title="pal">
          ...
          {sidebar && (
            <SettingsRow anchor={text.sidebar.anchor} label={text.sidebar.label} description={<>{text.sidebar.description}{onGo && <> <button type="button" className="pal-link" onClick={() => onGo("general", "general:sidebar")}>Sidebar settings</button></>}</>}>
