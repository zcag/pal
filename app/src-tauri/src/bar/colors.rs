...
impl Palette {
    ...
    /// The RGB triple, for the glyph rasteriser.
    pub fn rgb(&self, name: &str) -> Option<[u8; 3]> {
        self.argb(name).map(|c| [(c >> 16) as u8, (c >> 8) as u8, c as u8])
    }
}
...
mod tests {
    ...
    fn overrides_and_spellings() {
        ...
        assert_eq!(p.rgb("text"), Some([0xEC, 0xEC, 0xF0]));
